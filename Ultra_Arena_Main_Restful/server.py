...
def process_files():
    """
    Simple processing endpoint that uses default combo from profile configuration.
    ...
    Note: 
    - combo_name is not needed (uses DEFAULT_STRATEGY_PARAM_GRP from profile)
    - max_cc_strategies is automatically set to 1 (single strategy processing)
    - All other parameters work the same as /api/process/combo
    - Defaults can be overridden in profile_config.py
    """
    try:
        logger.info(f"🚀 STARTING /api/process request")
        
        # Start performance monitoring
        monitor = start_monitoring("REST_API_Process_Files")
        ...
        with time_operation("configuration_setup", "configuration_setup"):
            try:
                config = request_processor.create_unified_request_config(data, use_default_combo=True)
        ...
        logger.error(f"❌ ERROR in /api/process endpoint: {e}")
